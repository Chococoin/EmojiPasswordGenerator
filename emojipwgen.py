...
def automatic(args):
    charset = make_charset(args.use_upper, args.use_lower, args.use_digits, args.use_punctuation, args.use_space,
                           args.additional, args.blacklist, args.emoji)

    if not args.quiet:
        print(".----------------------------------------.")
        print("| 🌈💪😷 Emoji Password Generator 🇮🇹💗🇻🇪 |")
        print("| 🄯 2020 Chococoin Powered by Ethturin   |")
        print("*----------------------------------------*")
        print()
...
def interactive():
    ...
    charset = ask_charset(True, True, True, True, True, "", "", True)
...
def parse_args():
    ...
    description = """Highly customizable random password generator.
    ...
    parser.add_argument("-e", "--emoji", action="store_true", help="Use the emoji mode to generate passwords with emojis 😍")
...
def make_charset(use_upper, use_lower, use_digits, use_punctuation, use_space, additional, blacklist, use_emoji):
    if not all(isinstance(x, bool) for x in [use_upper, use_lower, use_digits, use_punctuation, use_space, use_emoji]) \
    ...
    return set(use_upper * string.ascii_uppercase +
               ...
               use_space * " " +
               use_emoji * string.emoji +
               additional) \
        .difference(set(blacklist))


def ask_charset(default_upper, default_lower, default_digits, default_punctuation, default_space,
                default_additional, default_blacklist, default_emoji):
    if not all(isinstance(x, bool) for x in
               [default_upper, default_lower, default_digits, default_punctuation, default_space, default_emoji]) \
    ...
    if ask_yn("Do you want to change the character set?", False):
        ...
        return make_charset(
            ask_yn("Do you want to allow emoji characters '{}'?".format(string.emoji), default_emoji),
